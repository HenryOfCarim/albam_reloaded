...
from .tools.tools import show_message_box, split_UV_seams_operator, select_invalid_meshes_operator, transfer_normals
...
class AlbamImportedItem(bpy.types.PropertyGroup):
    ...
    folder: bpy.props.StringProperty(
        options={'HIDDEN'})  # Always in posix format
...
class PasteCustomPropertiesMesh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for field in mesh_data._fields_:
            ...
            if not attr_name.startswith('unk_'):
                continue
            setattr(active_mesh.data, attr_name,
                    getattr(copied_mesh.data, attr_name))
        return {'FINISHED'}
...
class ALBAM_PT_ImportExportPanel(bpy.types.Panel):
    ...
    def draw(self, context):  # pragma: no cover
        ...
        layout.prop_search(scn, 'albam_item_to_export', scn,
                           'albam_items_imported', text='select')
        layout.operator('albam_export.item', text='Export')
        layout.prop(export_settings, "export_visible_bool",
                    text="Export visible meshes only")
        layout.prop(export_settings, "ignore_missing_mod_bool",
                    text="Ignore missing .mod files")
        layout.prop(export_settings, "clear_temp_foder_bool",
                    text="Clear temporary folder")
...
class ALBAM_PT_ToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout = self.layout
        row = layout.row()
        row.operator('albam_tools.fix_leaked_texures',
                     text="Fix leaked textures")
        row.prop(export_settings, "transfer_normals_bool", text="")
        layout.operator('albam_tools.select_invalid_meshes',
                        text="Select invalid meshes")
        layout.operator('albam_tools.remove_empty_vertex_groups',
                        text="Remove empty vertex groups")
        layout.operator('albam_tools.rename_bones', text="Auto-rename bones")
        layout.operator('albam_tools.transfer_normals',
...
class AlbamImportOperator(bpy.types.Operator):
    ...
    directory: bpy.props.StringProperty(
        subtype='DIR_PATH')  # fileselect_add properies here
    files: bpy.props.CollectionProperty(
        name='adf', type=bpy.types.OperatorFileListElement)  # fileselect_add properies here
    ...
    def _import_file(self, **kwargs):
        ...
        func = blender_registry.import_registry.get(
            id_magic)  # find header in dictionary
        if not func:
            raise TypeError(
                'File not supported for import. Id magic: {}'.format(id_magic))
...
class AlbamExportOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not func:
            raise TypeError(
                'File not supported for export. Id magic: {}'.format(id_magic))
